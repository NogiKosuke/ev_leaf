...
RSpec.describe Task, type: :model do
  ...
  describe 'バリデーションのテスト' do
    context 'タスクのタイトルが空の場合' do
      it 'バリデーションにひっかかる' do
        task = Task.new(title: '', content: '失敗テスト', expired_at: "#{3.day.from_now}", user: user)
        expect(task).not_to be_valid
      end
    end
    context 'タスクの詳細が空の場合' do
      it 'バリデーションにひっかかる' do
        task = Task.new(title: '失敗', content: '', expired_at: "#{3.day.from_now}", user: user)
        expect(task).not_to be_valid
      end
    end
    context 'タスクのタイトルと詳細に内容が記載されている場合' do
      it 'バリデーションが通る' do
        task = Task.new(title: '成功', content: '成功', expired_at: "#{3.day.from_now}", user: user)
  ...
  describe 'タスクモデル機能', type: :model do
    describe '検索機能' do
      ...
      let!(:task) { FactoryBot.create(:task, title: 'task', status: '実行中', user: user) }
      let!(:second_task) { FactoryBot.create(:second_task, title: "sample", status: '完了', user: user) }
      let!(:third_task) { FactoryBot.create(:third_task, title: "sample", status: '実行中', user: user) }
